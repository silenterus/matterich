...
from matterich.taps_function import sahi_autoseg_app
from matterich.taps_function import automask_image_app
from matterich.taps_function import automask_video_app_with_frames
from matterich.taps_function import extract_frames
...
def sahi_app():
    with gr.Blocks():
        with gr.Row():
            with gr.Column():
                sahi_image_file = gr.Image(type="filepath", height=260)
...
def video_app():
    with gr.Blocks():
        with gr.Row():
            with gr.Column():
                seg_automask_video_file = gr.Video(height=260)
            ...
            with gr.Column():
                ...
                output_frames = gr.Image(type="numpy")

        seg_automask_video_predict.click(
            fn=automask_video_app_with_frames,
            inputs=[
            ...
            outputs=[output_video, output_frames],
        )

        extract_frames_button.click(
            fn=extract_frames,
            inputs=[seg_automask_video_file],
